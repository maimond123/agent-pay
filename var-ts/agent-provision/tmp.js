...
var onCronTrigger = (runtime2) => {
  ...
  runtime2.log("Starting HTTP test...");
  runtime2.log(`URL: ${config.testUrl}`);
  const http = new ClientCapability;
  runtime2.log("Creating request...");
  const response = http.sendRequest(runtime2, {
    url: config.testUrl,
    method: "GET"
  }).result();
  runtime2.log(`Status: ${response.statusCode}`);
